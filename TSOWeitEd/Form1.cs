...
namespace TSOWeight
{
    public partial class Form1 : Form
    {
        ...
        private void lvSkinWeights_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (viewer.SelectedTSOFile == null)
                return;

            ListViewItem li = lvSkinWeights.SelectedItems[0];
            MqoSkinWeight skin_weight = li.Tag as MqoSkinWeight;

            viewer.SelectedNode = skin_weight.bone;
            Invalidate(false);
        }
        ...
        private void editResetPoseFromTSOToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Figure fig;
            if (viewer.TryGetFigure(out fig))
            {
        ...
        private void cameraSelectedVertexToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (viewer.TryGetFigure(out fig))
            {
                viewer.Camera.Center = viewer.SelectedVertex.CalcSkindeformPosition(fig.ClipBoneMatrices(viewer.SelectedTSOFile));
        ...
        {
            viewer.Source = viewer.SelectedVertex;
            Invalidate(false);
        }

        private void btnTarget_Click(object sender, EventArgs e)
        {
            viewer.Target = viewer.SelectedVertex;
            Invalidate(false);
        }
